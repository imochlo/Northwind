...
def get_completed_deliveries_dict():
    ...
    value = len(result)
    percentage = str(round((len(result)/len(order_detail_dim))*100, 2)) + "%"
    return {"value":value,
...
def get_amt_sold_over_time_bubble():
    ...
    for r in df["ShipRegion"].unique():
        ...
        trace = go.Scatter(
            ...
            name = r,
            mode='markers',
            marker=dict(size=0.001*result["TotalAmt"]),
...
def get_num_of_shipped_items_per_shipper_pie():
    ...
    layout = go.Layout(
        title = 'Number of Shipped Products <br>per Shipping Company', # Graph title
    )
    return go.Figure(data=[data], layout=layout)
...
def get_backorder_bar():
    ...
    df = df[["ProductName", "UnitsBackordered"]].sort_values(by='UnitsBackordered', ascending=True)
    data = go.Bar(y = df["ProductName"],
                  x = df["UnitsBackordered"],
                  orientation = "h",
                  text = df["UnitsBackordered"]
                  )

    layout = go.Layout(
        title = 'Number of Backordered Items <br>per Product', # Graph title
        xaxis = {"title": "Number of Backorders"},
    )
    return go.Figure(data=[data], layout=layout)
...
def get_reorder_bar():
    ...
    df["Delta"] = df[["ReorderLevel","UnitsInStock"]].apply(lambda vals: vals["ReorderLevel"]-vals["UnitsInStock"],axis=1)
    df["DeltaText"] = df[["Delta"]].apply(lambda vals: "Difference " + str(vals["Delta"]),axis=1)
    df = df[df["UnitsInStock"] < df["ReorderLevel"]].sort_values(by="Delta", ascending = True)

    in_stock = go.Bar(y = df["ProductName"], 
                      x = df["UnitsInStock"], 
                      name = "Units In Stock", 
                      marker = dict(color='rgb(234, 117, 0)', 
                                    opacity = 0.75), 
                      orientation = "h")
    reorder_level = go.Bar(y = df["ProductName"],
                           x = df["UnitSlack"],
                           name = "Reorder Level",
                           ...
                           orientation="h")
    layout = go.Layout(
        title = 'Products Under Reorder Level', # Graph title
        xaxis = {"title": "Reorder Ratio"},
        barmode = 'stack')
